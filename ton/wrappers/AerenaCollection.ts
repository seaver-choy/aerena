...
export class AerenaCollection implements Contract {
    constructor(
    ...
    static createFromAddress(address: Address) {
        return new AerenaCollection(address);
    }
    ...
    static createFromConfig(
        ...
        return new AerenaCollection(contractAddress(workchain, init), init);
    }
    ...
    async getItemAddr(provider: ContractProvider, nftId: number) {
        let { stack } = await provider.get("get_nft_address_by_index", [
            { type: "int", value: BigInt(nftId) },
        ]);
    ...
    static mintMessage(params: MintParams) {
        const uriContent = beginCell()
            .storeStringTail(params.contentUrl)
            .endCell();
    ...
    async sendMint(
        ...
        await provider.internal(via, {
            value: params.gas,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: AerenaCollection.mintMessage(params),
        });
    }
}
