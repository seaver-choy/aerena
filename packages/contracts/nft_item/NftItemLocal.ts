...
export class NftItemLocal {
    ...
    async getNftData(): Promise<NftDataResponse> {
        ...
        const [initialized, index, collectionAddressSlice, ownerAddress, content] = res.result as [BN, BN, Slice, Slice, Cell]

        const isInitialized = initialized.eqn(-1)
        ...
        const collectionAddress = collectionAddressSlice.readAddress()
        const isSingle = !collectionAddress
    ...
    async getEditor(): Promise<Address | null> {
        const res = await this.contract.invokeGetMethod('get_editor', [])
        if (res.type !== 'success') {
            return null
        }
        const [editorSlice] = res.result as [Slice]
        return editorSlice.readAddress()
    }

    async getRoyaltyParams(): Promise<RoyaltyParams | null> {
        const res = await this.contract.invokeGetMethod('royalty_params', [])
        if (res.type !== 'success') {
            return null
        }
        const [royaltyFactor, royaltyBase, royaltyAddress] = res.result as [BN, BN, Slice]
    ...
    async sendTransferEditorship(from: Address, params: { queryId?: number, newEditor: Address, responseTo: Address | null, forwardAmount?: BN }) {
        const msgBody = Queries.transferEditorship(params)
        return await this.contract.sendInternalMessage(new InternalMessage({
            to: this.address,
            from: from,
            value: toNano(1),
    ...
    static async createFromConfig(config: NftItemData) {
        const code = await compileFunc(NftItemSource)
    ...
    static async createSingle(config: NftSingleData) {
        const code = await compileFunc(NftSingleSource)

        const data = buildSingleNftDataCell(config)
        const contract = await SmartContract.fromCell(code.cell, data)
